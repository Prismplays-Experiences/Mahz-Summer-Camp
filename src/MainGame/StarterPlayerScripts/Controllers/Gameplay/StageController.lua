...
function CircularCameraMotion(Part, angleOffset, dist, duration)
	...
	local StartCF = CFrame.new(
		GetCurvePosition(60 + angleOffset, Part.Position.Y, mag) + newTargetPos - Vector3.new(0, newTargetPos.Y, 0),
		lookat
	)
	local EndCF = CFrame.new(
		GetCurvePosition(120 + angleOffset, Part.Position.Y, mag) + newTargetPos - Vector3.new(0, newTargetPos.Y, 0),
		lookat
	)
	local startcftween = TweenService:Create(
		cam,
...
function StageController:KnitStart()
	...
	local CurrentTune = "Normal"
	StageService.SetCamStatus:Connect(function(status, campart)
