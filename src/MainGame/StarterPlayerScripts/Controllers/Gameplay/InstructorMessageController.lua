...
function ZoomCameraFOV(sharpIn: number, holdTime: number, sharpOut: number, inFOV: number, outFOV: number)
    ...
    -- Zoom in
    local tweenIn = TweenService:Create(Camera, TweenInfo.new(sharpIn, Enum.EasingStyle.Sine, Enum.EasingDirection.In), {
        FieldOfView = inFOV
    })

    -- Zoom out
    local tweenOut = TweenService:Create(Camera, TweenInfo.new(sharpOut, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
...
function TypeText(textLabel, desiredtext, speed,SoundId)
...
function PlayMessage(Instructor,Message)
    ...
    TweenService:Create(TextHolder, 
    ...
    for i,MessageText in ipairs(Message) do
        for i,PhraseTable in KeyPhrases do
            for i,Phrase in PhraseTable.Text do
