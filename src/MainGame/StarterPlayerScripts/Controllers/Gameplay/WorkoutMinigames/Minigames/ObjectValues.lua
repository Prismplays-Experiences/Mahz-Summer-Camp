...
function ApplyTargetProperties(Text, Img)
	local UIScale = CurrentTarget:FindFirstChild("UIScale") or Instance.new("UIScale")
	UIScale.Parent = CurrentTarget
	UIScale.Scale = 1
...
function ObjectValuesMinigame:ThrowObject(targetFrame)
	local UIScale = targetFrame:FindFirstChild("UIScale") or Instance.new("UIScale")
	UIScale.Parent = targetFrame
	UIScale.Scale = 0

	local UIScaleTween = TweenService:Create(
		UIScale,
		TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out),
		{ Scale = 1 }
	)

	local UIScaleTweenOut = TweenService:Create(
		UIScale,
		TweenInfo.new(0.5, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out),
		{ Scale = 0 }
	)
