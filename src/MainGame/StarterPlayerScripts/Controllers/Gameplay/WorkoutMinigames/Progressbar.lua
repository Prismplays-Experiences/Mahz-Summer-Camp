...
function ProgressBarController:new(Bar)
    local self = setmetatable({}, ProgressBarController)
...
function ProgressBarController:ApplyScreenColorFlash(isCorrect, func)
    if self.ScreenFlashPlaying then return end
...
function ProgressBarController:SendPositiveMessage()
    if self.PositiveMessageDb then return end
    ...
    if not msg then self.PositiveMessageDb = false return end
...
-- end)
	...
	if self._progressThread then return end

	self._progressThread = task.spawn(function()
        repeat wait() until self.Incremented
...
function ProgressBarController:Stop()
    if not self._isUpdating then return end
