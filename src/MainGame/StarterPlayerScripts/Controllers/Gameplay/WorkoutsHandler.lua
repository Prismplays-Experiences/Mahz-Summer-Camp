...
function WorkoutsHandler:RunInjuryLogic()
    self.InjuryLogic:GetPropertyChangedSignal('Value'):Connect(function()
        if self.InjuryLogic.Value == false then return end
        self.GeneralService:GetDays():andThen(function( Days)
            local maxBase = 50
            ...
            for i = 1, InjuryTime do
                task.wait(1)
                if not self.InjuryLogic.Value then return end
                if i >= InjuryTime then self.Injured = true end
            end
            if not self.Injured then return end
            self:StopWorkout()
...
function WorkoutsHandler:ControlProximityPrompts(Enabled)
    if Enabled and self.Injured then return end
    local CurrentDay
    self.GeneralService:GetDays():andThen(function(val) 
        CurrentDay = val
    end):await()
...
function WorkoutsHandler:StartWorkout(slot,Data)
    self.InWorkout = true
    if slot:GetAttribute('Owner') then return end
    ...
    if Data.Tool then
        self.GeneralService:GiveTool(Data.Tool):andThen(function(Tool)
    ...
    self.FailedEvent = self.MinigameData.Failed:Connect(function(boolean)
        if self.Injured then return end
...
function WorkoutsHandler:LoseWeight(Value)
    ...
    ShortNotification(`-{Value} lbs`, Color3.fromRGB(255, 209, 57), true)
    self.GeneralService:LoseWeight(Value)
end

function WorkoutsHandler:StopWorkout()
    if not self.InWorkout then return end
        ...
        for index,slot in workout:GetChildren() do
            ...
            ProximityPrompt.Triggered:Connect(function()
                if self.MachineLocked then
                    ...
                    SendNotification('Workout Machines are locked!', Color3.fromRGB(255, 0, 0), 2, false, SoundEffects.UIDeny)
                    return
                end
                if self.Injured then SendNotification('You are injured! Recover first!', Color3.fromRGB(255, 0, 0), 2, false, SoundEffects.UIDeny) return end
                if slot:GetAttribute('Owner') then
                    SoundEffects.UIDeny:Play()
                    return
                end
                if self.Cooldown then SoundEffects.UIDeny:Play() return end
                self.Cooldown = true
...
function WorkoutsHandler:KnitStart()
    ...
    if CheckIfAlive(Player) then CharacterAdded(Player.Character) end
    
    Player.CharacterAdded:Connect(CharacterAdded)

    self.GeneralService.UpdateSlots:Connect(function(Slots)
        if self.CurrentTrack then return end
    ...
    for i,billboard in WorkoutBillboards:GetChildren() do
        if GeneralInfo.WorkoutStartDays[billboard.Name].Day<2 then continue end
        ...
        BuyNow.MouseButton1Click:Connect(function()
            if not UnlockIndex then return end
