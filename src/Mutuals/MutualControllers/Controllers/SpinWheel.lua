...
local WheelSpinnerContents = require(Modules:WaitForChild('Client')
                                    :WaitForChild('Rewards')
                                    :WaitForChild('WheelSpinnerContents'))
...
function SpinsController:KnitStart() --KnitStart()
    ...
    for _, v in pairs(SpinWheel:GetChildren()) do
        if v:IsA("Folder") then
            v.Percent.Text = tonumber(Chances[tonumber(v.Name)]) .. "%"
        end
    end
    ...
    Spinbtn.MouseButton1Click:Connect(function()
        ...
        if SpinsStat.Value > 0 then
            WheelService:ValidateWheelSpinner("check"):andThen(function(Status, errorMessage)
                if not Status then
            ...
            if Prize then
                WheelService:ValidateWheelSpinner("reward", Prize):andThen(function()
                    local reward = WheelSpinnerContents["WheelValues"][Prize]
                    -- if reward then
                        -- local notification = string.format("You won: %s", reward.Name)
                        -- SendNotification(notification, Color3.fromRGB(0, 239, 0), 2, true)
                    -- else
                    --     warn("Invalid prize index:", Prize)
                    -- end
                end):catch(function(err)
