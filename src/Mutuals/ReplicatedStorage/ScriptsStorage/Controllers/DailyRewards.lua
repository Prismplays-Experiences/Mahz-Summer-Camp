...
local function splitToSentence(inputString)
	local words = {}
	local wordStart = 1
	for i = 2, #inputString do
		if inputString:sub(i, i):match("%u") then
			table.insert(words, inputString:sub(wordStart, i - 1))
			wordStart = i
		end
	end
	table.insert(words, inputString:sub(wordStart))
...
function DailyRewardController:Blub()  -- KnitStart()
	...
	Player:FindFirstChild("DataLoaded") and Player.DataLoaded.Value
		...
		if btn:IsA("GuiButton") then
			...
			btn.MouseButton1Click:Connect(function()
				...
				if btn.CanClaim.Value then
					...
					local notifyText = tonumber(btn.Name) <= Streak.Value and "Already Claimed, come back tomorrow!" or "Claim this next time!"
