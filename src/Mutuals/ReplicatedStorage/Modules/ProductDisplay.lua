...
local function SpinAndBob(Model, height, spinSpeed)
	...
	Connection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		Model:PivotTo(pivotCFrame * CFrame.new(0, bobOffset, 0) * CFrame.Angles(0, angle, 0))
	end)
...
local function Bob(Model, height, bobSpeed)
	...
	Connection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		Model:PivotTo(pivotCFrame * CFrame.new(0, bobOffset, 0))
	end)
...
local function Spin(Model, spinSpeed)
	...
	Connection = RunService.Heartbeat:Connect(function(deltaTime)
		...
		Model:PivotTo(pivotCFrame * CFrame.Angles(0, angle, 0))
	end)
...
function NewProductDisplay(model, id, itemType, movement)
	...
	if itemType == "Product" then
		...
		_price = marketInfo.PriceInRobux
	else
